...
- **Target mobile-chrome specifically only when the ticket itself signals mobile-specific behavior.** \`playwright.config.ts\` always ships both \`chromium\` (desktop) and \`mobile-chrome\` (\`devices['Pixel 5']\` emulation) as active projects. By default, Stage 5 (Implement) runs against \`chromium\`. Read the scenario/ticket content at Stage 5 time: if it clearly describes mobile-specific behavior — it mentions "mobile", "responsive", touch gestures (tap/swipe/pinch), a mobile viewport, or is explicitly tagged/labeled as a mobile test — target \`mobile-chrome\` instead (e.g. run/scope the spec with \`--project=mobile-chrome\`, or an equivalent \`@mobile\` tag), rather than running both by default. This is a judgment call made by reading the ticket, not a separate question to the user — only ask if the ticket is genuinely ambiguous about which platform it's targeting.
