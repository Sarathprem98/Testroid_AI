...
If a user's message contains a test scenario description, an epic reference (e.g. \`EPIC-123\`), a Jira ticket ID or link, a SPEC file, or hand-written test cases, treat it as likely input to the Testroid agent pipeline described above — not something to turn into ad hoc tests or code immediately.

- **Confirm before starting.** Ask something like: "This looks related to the Testroid pipeline — should I run it through the full pipeline (Test Plan Generator → Test Case Generator → Normalizer → Reuse Matcher → Implement / API Automator / Mobile Automator → Validator), or just a specific stage?" Don't start drafting a test plan, test cases, or code before the user answers.
- **Only proceed on explicit confirmation.** Invoke the Pipeline Orchestrator (\`docs/agents/PipelineOrchestratorAgent.md\`) — which drives Stage 1 through Stage 6, pausing at the HITL gates in \`docs/agents/README.md\` — only once the user has explicitly said to run the full pipeline, never by inference from the input alone.
- **Respect a request for a single stage.** If the user says they only want one stage — e.g. "just generate the test plan," "just normalize these," or "check what's already covered" — run only that stage via its own doc in \`docs/agents/\` (\`TestPlanGeneratorAgent.md\`, \`TestCaseGeneratorAgent.md\`, \`TestCaseNormalizerAgent.md\`, \`ReuseMatcherAgent.md\`, \`ImplementAgent.md\`/\`ApiAutomatorAgent.md\`/\`MobileAutomatorAgent.md\`, or \`ValidatorAgent.md\`), and don't silently continue into the next stage afterward without asking again.
- **Manually authored test cases still confirm.** If the input is already a set of hand-written test cases rather than a scenario/epic/ticket, the same rule applies before feeding them into Stage 3 (Test Case Normalizer Agent) as the pipeline's alternate entry point (see \`docs/agents/README.md\`'s "Alternate Entry Point: Manual Test Cases").
